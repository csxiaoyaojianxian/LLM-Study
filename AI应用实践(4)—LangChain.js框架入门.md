...
GitHub 仓库：[https://github.com/csxiaoyaojianxian/LLM-Study/tree/main/05-langchain](https://github.com/csxiaoyaojianxian/LLM-Study/tree/main/05-langchain)
...
- **聊天应用阶段**：手动调用 API、手动解析 SSE 流、手动维护 `history` 数组实现多轮对话
- **Prompt 工程阶段**：自己封装 `model-adapter.ts` 做多模型适配（~80行）、手动字符串拼接 Prompt
- **RAG 实战阶段**：手写三种文本分块策略（~120行）、手动封装 ChromaDB 客户端（~100行）、手写完整 RAG Pipeline（~150行）
...
对比前文自己封装的 `model-adapter.ts`（~80行），LangChain 的 `ChatOpenAI` 开箱即用，一行 `baseURL` 搞定适配。
...
回顾前面的聊天应用实现，我们手动解析 SSE 流，处理 `data:` 前缀、`[DONE]` 标记，代码繁琐。LangChain 一行搞定：
...
还记得前文 Prompt 工程里我们怎么做的吗？

```typescript
// ❌ 前文方案：手动拼接 Prompt → 调用模型 → 手动解析输出
...
回顾前文，我们用 Vercel AI SDK 的 `generateObject()` + Zod Schema 实现结构化输出。LangChain 的方案是 **Output Parser + `format_instructions`**——在 Prompt 里告诉 LLM "以什么格式输出"，然后用 Parser 解析：
...
这个问题在前面的聊天应用里就提到过——**LLM 是无状态的，每次 API 调用都是全新的、独立的**。它不会"记住"你上一轮说了什么。
...
这是前文用过的最基础方式，只是换成了 LangChain 的消息类型：
...
前面的聊天应用里，我们在 Vercel AI SDK 中用 Zod Schema + `tools` 对象定义了天气查询和计算器工具。LangChain 的方式类似，但用 `tool()` 函数封装：
...
这是本篇的压轴环节：用 LangChain 组件重新实现前文手写的 RAG Pipeline，直观感受框架带来的效率提升。

### 7.1 手写 vs LangChain 方案对比

| 环节 | 前文手写 | 本篇 LangChain |
|------|----------|---------------|
...
前文里我们手写了三种分块策略（固定大小、递归字符、段落），约120行。LangChain 的 `RecursiveCharacterTextSplitter` 配置一下就搞定：
...
LangChain 的向量化需要实现 `Embeddings` 抽象类。这里复用前文的 `@xenova/transformers` 本地模型（`Xenova/bge-small-zh-v1.5`，512维，中文优化），无需 API Key：
...
前文手动封装 ChromaDB 客户端约 100 行。LangChain 直接提供 `Chroma` 集成：
...
对比前文手写的 `RAGPipeline` 类（~150行 Pipeline + 手动拼接 Prompt + 手动调用模型），这里不到 20 行代码实现了完整的 RAG 链，而且数据流一目了然。
...
| 概念 | 作用 | 关键类/函数 | 对应前几期 |
|------|------|-----------|-----------|
| **Model** | 统一接口调用不同 LLM | `ChatOpenAI`（baseURL 适配 DeepSeek） | 前文 `model-adapter.ts` |
| **Prompt** | 类型安全的模板引擎 | `ChatPromptTemplate`、`MessagesPlaceholder` | 前文手动字符串拼接 |
| **LCEL** | 管道式链式调用 | `pipe()`、`RunnableSequence`、`RunnableLambda` | — |
| **Output Parser** | 结构化输出解析 | `StructuredOutputParser` + Zod | 前文 `generateObject()` |
| **Memory** | 自动管理对话历史 | `RunnableWithMessageHistory`、`MemorySaver` | 前文手动 history 数组 |
| **Tool + Agent** | 工具调用 + 自动编排 | `tool()`、`bindTools()`、`createReactAgent` | 前文 Function Calling |
| **LCEL RAG Chain** | 完整 RAG Pipeline | `Retriever.pipe(formatDocs)`... | 前文手写 RAG |
