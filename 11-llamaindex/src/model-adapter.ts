/**
 * model-adapter.ts — 多模型统一适配层（扩展版，新增 Ollama）
 *
 * 在原有 deepseek / openai / anthropic 基础上新增 ollama provider。
 * Ollama 通过 OpenAI 兼容 API 接入，无需额外包。
 *
 * 扩展自 07-mcp。
 */
 ...
 * @param provider - 模型提供商（deepseek / openai / anthropic / ollama）
  ...
  switch (provider) {
    ...
    case "ollama": {
      ...
      const ollama = createOpenAI({
        ...
        apiKey: "ollama", // Ollama 不需要真实 Key，但字段必填
      });
      return ollama(model);
    }
    default:
