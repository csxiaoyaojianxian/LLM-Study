...
前面第 3 篇手写了完整的 RAG 系统，第 4 篇用 LangChain 重构了一遍。两次实践下来，RAG 的每个环节（切分、向量化、检索、生成）都已经摸透。但如果站在"知识管理"的角度重新审视，会发现这两套方案都偏"底层拼装"——你需要自己选切分器、自己接向量库、自己组装 Prompt。有没有更专注于数据索引和检索的方案？
...
| 实现方式 | 模块 | 代码量 | 特点 |
|---------|------|--------|------|
| 手动实现 | 第 3 篇 | ~200 行 | 原理清晰，但重复代码多 |
| LangChain | 第 4 篇 | ~100 行 | LCEL 组合灵活，通用性强 |
| LlamaIndex | 本篇 | ~50 行 | 专注索引检索，开箱即用 |

> 🎒 **类比**：想想 Web 开发中处理数据库的三种方式：
> - **手写 SQL** = 第 3 篇（控制力最强，但样板代码多）
> - **通用 ORM（Sequelize）** = 第 4 篇 LangChain（灵活，能做任何事）
> - **专用查询构建器（Prisma）** = 本篇 LlamaIndex（专注数据层，体验更好）
>
...
| LlamaIndex | 第 3 篇（手动） | 第 4 篇（LangChain） |
|------------|------------------|----------------------|
...
Document 是 LlamaIndex 中原始数据的抽象。不同于第 3 篇里 `fs.readFileSync()` 返回的纯字符串，Document 是一个结构化对象，包含文本内容和元数据：
...
> 💡 对比第 3 篇：第 3 篇的"文档"就是 `fs.readFileSync()` 返回的字符串，元数据（文件名、来源等）需要自己用单独的变量或对象维护。LlamaIndex 的 Document 把文本和元数据封装在一起，后续切分成 Node 时元数据会自动继承，不用手动传递。

### 2.2 Node——切分后的片段

Node（TextNode）是 Document 经过切分后的最小单元，相当于第 3 篇里的 chunk。LlamaIndex 使用 `SentenceSplitter` 完成切分：
...
| 特性 | 第 3 篇手动切分 | LlamaIndex SentenceSplitter |
|------|-------------------|---------------------------|
...
> 💡 对比第 2 篇的 `model-adapter.ts`：我们自己封装的适配层基于 Vercel AI SDK，用 `getModel(provider)` 返回统一接口。LlamaIndex 的 Settings 是框架内置的全局配置，基于自己的 LLM 封装。两者解决的是同一个问题——多模型适配，但实现路径不同。
...
```
Query(用户问题) → Retriever(检索TopK) → NodePostprocessor(重排序/过滤) → ResponseSynthesizer(上下文+LLM生成) → Answer(最终回答)
```
...
对比第 3 篇：手动实现是 `results.filter(r => r.score > threshold)`，原理完全一样，LlamaIndex 只是把它标准化为 `NodePostprocessor` 接口。
...
路由决策的底层原理是：RouterQueryEngine 把所有工具的 `description` 和用户问题一起发给 LLM，让 LLM 决定用哪个工具。这和第 5-6 篇中 Multi-Agent 的 Routing 模式本质相同——都是让 LLM 做"调度员"。
...
> 💡 RouterQueryEngine 的 `description` 字段至关重要。描述写得模糊，LLM 就分不清该路由到哪个引擎。这和第 5-6 篇中定义 Tool description 的经验一致——**给 LLM 的指令越明确，行为越可预测**。
...
对比第 3 篇 `rag-optimize.ts` 中的 HyDE（Hypothetical Document Embedding）：
...
class LlamaIndexRAG {
  ...
  /** 构建索引 — 对应第 3 篇的 chunk + embed + store 三步 */
  async buildIndex(): Promise<void> {
    // LlamaIndex 一步到位！
    this.index = await VectorStoreIndex.fromDocuments(this.documents);
  }

  /** RAG 查询 — 对应第 3 篇的 RAGPipeline.query() */
...
和第 3 篇的 `RAGPipeline` 对比，关键差异在 `buildIndex()`。第 3 篇需要分别调用 `chunk()`、`embed()`、`store()` 三个函数，每个都要自己管理参数和错误处理。LlamaIndex 的 `fromDocuments()` 把这三步封装成了一个原子操作。
...
第 3 篇的 `conversational-rag.ts` 用问题改写解决代词问题（约 50 行代码）——当用户说"它的泛型有什么用"时，需要把"它"替换为"TypeScript"再去检索。LlamaIndex 的 `ContextChatEngine` 内置了这个能力：
...
`ContextChatEngine` 内部自动维护对话历史，每次查询时结合历史上下文做问题理解和检索。第 3 篇手动实现这个功能需要：维护 `chatHistory` 数组 → 构造改写 Prompt → 调用 LLM 改写问题 → 用改写后的问题检索。ContextChatEngine 把这些都封装了。
...
| 环节 | 第 3 篇手写 | 第 4 篇 LangChain | 本篇 LlamaIndex |
|------|---------------|--------------------|--------------------|
| 文档加载 | 5 行 | 5 行 | 10 行 |
| 切分 | 40 行 | 5 行 | —（自动） |
| 向量化 | 30 行 | 5 行 | —（自动） |
...
```
你的需求是什么？
├── 需要理解 RAG 底层原理 → 第 3 篇手动实现
│   （学完再上框架，知其所以然）
├── 构建通用 LLM 应用 → 第 4 篇 LangChain
│   （Agent + Chain + Memory 全家桶）
├── 专注知识管理和文档 QA → 本篇 LlamaIndex
│   （最少代码实现最完整的 RAG）
